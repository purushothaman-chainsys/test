	...
	$.fn.columnManager = function(options)
	{
		...
		var createColumnHeaderList = function(table)
		{
			...
			for ( var i = 0; i < cells.length; i++ )
			{
				...
				//alert('column :::::::::::::::::: '+ text + ' cols visible '+ colsVisible[i] );
				$li[0].cmData = {id: table.id, col: i};
				$list.append($li);
			}
			buildMyColumns11(cells, colsVisible);
//			alert('$list html ' + $list[i]);
...
var buildMyColumns11 = function(something, colsVisible) {

$.each(something, function(index, value) { 
//  	alert(index + '::::::::::::::::::: ' + $(this).html() + something.length);   	
});
	var xxx = 'manoj';
	
	
					var option = { width: 150, items: [
                            { text: "Print", icon: contextPath+"/images/contextmenu/ac0036-16.gif", alias: "1-1", action: menuAction },
//                            { text: "Toggle Columns", icon: "/APMWeb/images/contextmenu/ac0036-16.gif", alias: "1-2", action: menuAction }, 
//                            { text: "Copy", icon: "/APMWeb/images/contextmenu/ei0021-16.gif", alias: "1-3", action: menuAction },
                            { type: "splitLine" },
//                          { text: "Hide Columns", icon: "/APMWeb/images/contextmenu/wi0009-16.gif", alias: "1-4", type: "group", width: 170, items: [	                      
//                            ]
//                          },
//                            { type: "splitLine" },
                            { text: "Filter", icon: contextPath+"/images/contextmenu/wi0124-16.gif", alias: "1-5", type: "group", width: 100, items: [
	                             { text: "On", icon: contextPath+"/images/contextmenu/wi0062-16.gif", alias: "1-5-1", action: menuAction },
	                             { text: "Off", icon: contextPath+"/images/contextmenu/wi0062-16.gif", alias: "1-5-3", action: menuAction }
	                             //, { text: "Clear", icon: "/APMWeb/images/contextmenu/wi0062-16.gif", alias: "1-5-3", action: menuAction }
	                           ]
	                        }
                            ], onShow: applyrule,
  ...
  //              	toggleColumns(true);
	               ...
	               // toggleColumns(true);
   	                ...
   	                //toggleClick((this.data.alias).substr(2));
	            }
	            else if( (this.data.alias).indexOf('1-5-1')>=0) {
		            $('div#filtertarget').show(300);
	            }
	            else if( (this.data.alias).indexOf('1-5-2')>=0) {
		            $('div#filtertarget').hide(300);
	            }
	            else if( (this.data.alias).indexOf('1-5-3')>=0) {
		            $('div#filtertarget').hide(300);
	            	filter2('clear');
	            }
            }
			...
			$.each(something, function(index, value) {
  				if ( $.inArray(index, settings.hideInFilterList) < 0 ) {  					
  					var encoded = $(this).html();
  					var decoded = $("<div123/>").html(encoded).text();  					
					$('#mySelect').append($("<option></option>").attr("value",index).text( decoded)); 
				}
			});
...
var buildMyColumns11_backup12062012 = function(something, colsVisible) {
...
//	  	alert(index + '::::::::::::::::::: ' + $(this).html() + something.length);   	
	 ...
	 var option = { width: 150, items: [
	                            { text: "Print", icon: contextPath+"/images/contextmenu/ac0036-16.gif", alias: "1-1", action: menuAction },
	                            { text: "Toggle Columns", icon: contextPath+"/images/contextmenu/ac0036-16.gif", alias: "1-2", action: menuAction }, 
	                            { text: "Copy", icon: contextPath+"/images/contextmenu/ei0021-16.gif", alias: "1-3", action: menuAction },
	                            { type: "splitLine" },
	                            { text: "Hide Columns", icon: contextPath+"/images/contextmenu/wi0009-16.gif", alias: "1-4", type: "group", width: 170, items: [	                      
	                            ]
	                            },
	                            { type: "splitLine" },
	                            { text: "Filter", icon: contextPath+"/images/contextmenu/wi0124-16.gif", alias: "1-5", type: "group", width: 100, items: [
...
//	            alert('option.items[1].text '+ option.items[1].text)
//				option.items[5].items[1].text = 'Manoj Column1';
				
				$.each(something, function(index, value) { 
//					alert('visible '+ colsVisible[index]);
//	  				alert(index + '::::::::::::::::::: ' + $(this).html());   
//	  				option.items[5].items[index].text = $(this).html();	
	  				option.items[4].items[index] = new Object();
	  				option.items[4].items[index].text = $(this).html();	
//	  				alert($(this).className);
	  				option.items[4].items[index].icon = contextPath+"/images/contextmenu/wi0009-16.gif";
//	  				option.items[4].items[index].icon = "/lspg41/APMWeb/images/contextmenu/wi0124-16.gif";
	  				option.items[4].items[index].alias = "2-"+index;
	  				option.items[4].items[index].action = menuAction;
	  				
//	  				if ( $.inArray(index, settings.hideInFilterList) >= 0 )
	  				if ( $.inArray(index, settings.hideInFilterList) < 0 )  				
					{
						$('#mySelect').append($("<option></option>").attr("value",index).text($(this).html())); 
					}
		 ...
		 */
		var toggleClick = function(cNum)
		{
			//alert('entring toggle click xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx' +cNum);
			//get table id and column name
			var data = {id: 'mainData', col: cNum};//this.cmData;
			data.col = cNum;
						
			if ( data && data.id && data.col >= 0 )
			{
	//					alert('IFFFFFFFFFFFFFFF entring toggle click xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx' +cNum);
				var colNum = cNum; //data.col; 	
				var $table = $('#' + data.id);					
//alert('colNum ' + colNum);					
					
				if ( $table.length )
				{
		//		alert('TTTTTTTTTTTTTTTTTTTTT ABLE LENGTH toggle click xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx' +$table.length);
					$table.toggleColumns([ new Number(colNum) + 1], settings);
					//set the appropriate classes to the column header list
					var colsVisible = $table.get(0).cMColsVisible;
					if ( settings.onToggle )
					{
			//		alert(' if ( settings.onToggle )click xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx' +$table.length);
						settings.onToggle.apply($table.get(0), [colNum + 1, colsVisible[colNum]]);
					}
//					buildMyColumns11($table.tHead.rows[0].cells);
				}
			}
...
//			$('div#targetone').css('display','none');
		//	$('div#targetone').toggle(200);
//		alert('TTTTTTTTTTTTTTOOOOOO ccccccccclick end COMPLETED')
		};
...
//        alert('create column heaers');
	...
	//alert('EEEEEEEEEEnter ing toggleColumns '+ columns + 'cmo '+ cmo)
...
//			columnManager.buildMyColumns11(columns, colsVisible);
	...
	$.fn.alertMe = function() {
//		alert('MMMMMMMMMMMMMMMMMMMMMMa noj');
	};
	
	$.fn.togglePrepColumn = function(cNum)
		{
			toggleColumns(cNum);
			...
			if ( data && data.id && data.col >= 0 )
			{
			alert('entring toggle click xdddddddddddddddddata  $table.length '+$table.length);
				var colNum = cNum; //data.col, 	$table = $('#' + data.id);										
				if ( $table.length ) {
					$table.toggleColumns([colNum + 1], settings);
					//set the appropriate classes to the column header list
					var colsVisible = $table.get(0).cMColsVisible;
