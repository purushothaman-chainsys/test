		...
		$("input[name=selectedIndex]:checked").each(function(){
			...
			if($.trim($("input[name=workId]:eq("+checkedIndex+")").val()) == '') {
				...
				if(isNaN(wId) || Number(wId) <=0 || !allNumeric(wId)) {
					workIdNumeric = false;
					errMsg += '<li><attribute spring:message code="us.error.apm.archives.workid.nonnumber" arg0="'+wId+'"/></li>';
				}
			}
...
<input type ="hidden"  name="assignedToUser"  value ="${apmPerformanceBulkRequest.assignedToUser}" Id='"assignedToUser_"+${currentIndexId.index} '/>
