...
function assignUsers(assignType) {
			...
			if(assignType === 'ASSIGN') {
				...
				if($.trim(val) === '') {
					$("#serverErrorMessages").html("");
					$("#errorMessages").html("");
					$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.perfheader.update.asgusrid.value"/></span>');
			 		location.href="#"; 
			 		return;
				}
			...
			if($("table#mainData tr input[name='selectedIds']:checked").length > 0) {	
	    		...
	    		$("#serverErrorMessages").html("");
				$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.perfheader.update.asgusrid.selection"/></span>'); 
				location.href="#";
				return;
	    	}
}
