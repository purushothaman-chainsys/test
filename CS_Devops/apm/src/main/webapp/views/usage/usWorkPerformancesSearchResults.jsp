...
$(function() {
		...
		/* 
			...
			if(useTypeChanged) {			
				...
				$("#serverErrorMessages").html("");
				$("#uierror").html('<span class=txtRed><li><spring:message code="us.message.apm.workperf.mult.update.nochange"/></span>'); 
				location.href="#";
		...
		if($("table#mainData tr input[name='selectedIds']:checked").length > 0) {			
       	...
       	$( "#dialog:ui-dialog" ).dialog( "destroy" );
 			$( "#dialog_Delete" ).dialog({
				...
				$("#serverErrorMessages").html("");
				$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.perfheader.delete.selection"/></span>'); 
				location.href="#";
				return;
			}
    	});
    	...
    	if($("table#mainData tr input[name='selectedIds']:checked").length > 0) {    		
			...
			$("#uierror").html("");
 			...
 			$( "#dialog_Move" ).dialog({
				...
				$("#serverErrorMessages").html("");
				$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.perfheader.move.selection"/></span>'); 
				location.href="#";
				return;
			}
    	});
...
function changeWrkPerfAction(operation) {
	...
	if('DELETE_WORK_PERFORMANCE' == operation || 'MOVE_WORK_PERFORMANCE' == operation) {
		document.forms[0].action = '<%=request.getContextPath()%>/usage/workPerformancesListMultiAction';
...
function changeWrkPerfMedleyAction(operation, obj) {
	...
	document.forms[0].action = '<%=request.getContextPath()%>/usage/workPerformanceAction';
	document.forms[0].actionSelected.value = operation;
	document.forms[0].submit();
