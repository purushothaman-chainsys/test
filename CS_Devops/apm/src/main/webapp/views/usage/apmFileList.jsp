...
<script type="text/javascript">
	...
	function update(obj) {
		if ($("table#mainData tr input:checked").length == 0) {
			...
			$("#uierror")
					.html(
							'<span class=txtRed><li><spring:message code="us.error.apm.archives.update.selection"/></span>');
			unCheckSub(obj);
			location.href = "#";
			return;
		} else {
					...
					<c:forEach varStatus="currentIndexId" var="apmFile"
						...
						<tr align="center">
							...
							<td><form:input
									...
									id="yearquarterpicker_targetYearQuarter_${currentIndexId.index}"
									size="8" value="${apmFile.targetYearQuarter}" disabled="true" />
								<a
								href="javascript:clearField('<%="yearquarterpicker_targetYearQuarter_"%>${currentIndexId.index}');"
