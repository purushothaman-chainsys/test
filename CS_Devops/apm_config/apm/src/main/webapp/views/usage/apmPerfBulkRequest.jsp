		...
		$("input[name=selectedIndex]:checked").each(function(){
			...
			if($.trim($("input[name=workId]:eq("+checkedIndex+")").val()) == '') {
				...
				if(isNaN(wId) || Number(wId) <=0 || !allNumeric(wId)) {
					workIdNumeric = false;
					errMsg += '<li><spring:message code="error.apm.archives.workid.nonnumber" arguments="'+wId+'"/></li>';
				}
			}
			if( $.trim(  $("input[name=multWorkId]:eq("+checkedIndex+")").val()) != '') {
...
<form:input path="workId" maxlength="11" size="10" styleClass="redError" id='"wId_"+${currentIndexId.index}'/>	
</c:if>
<c:if  test="${apmPerformanceBulkRequest.invalidWorkId ne 'Y'}"> 
<input type="text" name="workId"  maxlength="11" size="10" id='"wId_"+${currentIndexId.index}'/>
</c:if>
</td>
<c:if test="${apmPerformanceBulkRequestList.filterResultViewType eq 'SPLR'}">
...
<c:out value="${apmPerformanceBulkRequest.estimatedDollarAmount}" /></td>
...
<form:hidden path="originalWorkId[${currentIndexId.index}]" value="${apmPerformanceBulkRequest.originalWorkId}" styleId='"originalWorkId_"+${currentIndexId.index}'/>
