	...
	$('input[name=selectPercent]').each(function() { 
		...
		if(sPercent != '') {
			if(isNaN(sPercent) || sPercent < 0 || Number(sPercent) <= 0) {
				errorsFound = true;	
				if(!sPercentError)	{	
					errMessage += '<li><spring:message code="error.apm.sampling.startskipselect.percent.numeric"/>';
				}	
				sPercentError = true;	
			}
				...
				errorsFound = true;	
				if(!sPercentError)	{	
					errMessage += '<li><spring:message code="error.apm.sampling.startskipselect.percent.gt100"/>';
				}	
				sPercentError = true;
			}
		}		
		if(sCount != '') {
				...
				errorsFound = true;
				if(!sCountError) {			
					errMessage += '<li><spring:message code="error.apm.sampling.startskipselect.count.numeric"/>';
				}	
				sCountError = true;		
			}
			else {
				if(sCount > totCount) {
					errorsFound = true;
					if(!sCountError) {			
						errMessage += '<li><spring:message code="error.apm.sampling.numberOfPerformancesToBeSampled.gt"/>';
					} sCountError = true;
				}
			}
		}		
		if(sStart != '') {
			if(isNaN(sStart) || !allNumeric(sStart) || Number(sStart) <= 0) {
				errorsFound = true;
				if(!sStartError) {			
					errMessage += '<li><spring:message code="error.apm.sampling.startskipselect.start.numeric"/>';
				} sStartError = true;		
			}
			else {
				if(sStart > totCount) {
					errorsFound = true;
					if(!sCountError) {			
						errMessage += '<li><spring:message code="error.apm.sampling.startskipselect.start.gt"/>';
					} sCountError = true;
				}
			}
		}
		if(sSkip != '') {
			if(isNaN(sSkip) || !allNumeric(sSkip) || Number(sSkip) <= 0) {
				errorsFound = true;
				if(!sSkipError) {			
					errMessage += '<li><spring:message code="error.apm.sampling.startskipselect.skip.numeric"/>';
				}	
				sSkipError = true;		
			}
		}
		...
		if(sPercent == '' && sCount == '' && sStart == '' && sSkip == '')  {
			errorsFound = true;				
			if(!combinationError) {
				errMessage += '<li><spring:message code="error.apm.sampling.startskipselect.invalid" arguments="'+(myIndex+1)+'"/>';
			}
...
function c2l2(thisObj) {
	...
	if(wrkPerfCount == '') {
		errorsFound = true;
		errMessage += '<li><spring:message code="error.apm.sampling.numberOfPerformancesToBeSampled.required"/>';
	}
	else {		
		    //if(!allNumeric(wrkPerfCount) ||  isNaN(wrkPerfCount) || Number(wrkPerfCount) <= 0) {		 
			if(!allNumeric(wrkPerfCount) ||  isNaN(wrkPerfCount) ) {		
			errorsFound = true;
			errMessage += '<li><spring:message code="error.apm.sampling.numberOfPerformancesToBeSampled.numeric"/>';
		}
		else  {		
	...
	if(countCensus != 1) {
		errorsFound = true;
		errMessage += '<li><spring:message code="error.apm.sampling.censusIndicator.required"/>';
	}
	...
	var mergeValIndex=0;var mergeErrorRows = false;var mergeErrorSurvInd = false;var mergeErrorFrnFlag = false;
		...
		if(currRowVal != '' && currRowVal != 'undefined') {
			...
			if(currRowVal != prevRowVal && currRowVal != nextRowVal) {errorsFound = true;
				if(!mergeErrorRows){errMessage += '<li><spring:message code="error.apm.sampling.mergeIndicator.invalid" arguments="'+currRowVal+'"/>';}mergeErrorRows = true;
			}
		}		
	});
    	...
    	if (mergeValArray[i + 1] == mergeValArray[i]) {
        	if (  $('select._includeForeignAffFlag:eq('+mergeArray[i]+')').val() !=  $('select._includeForeignAffFlag:eq('+mergeArray[i+1]+')').val()) {        	
        		...
        		if(!mergeErrorFrnFlag) {
					errMessage += '<li><spring:message code="error.apm.sampling.mergeIndicator.frn.invalid"/>';
				}
        	...
        	if(censusIndicatorArray[mergeArray[i]] !=  censusIndicatorArray[mergeArray[i+1]]) {
        		...
        		if(!mergeErrorSurvInd) {
					errMessage += '<li><spring:message code="error.apm.sampling.mergeIndicator.census.invalid"/>';
				}
