...
$(function() {
	...
	$('#_Submit').click(function() {
		...
		if($('#_exemptionType').val() == '') {
			errorsFound = true;
			errMsg += '<li><spring:message code="field.errors.generic.required" arguments="Exemption Type"/></li>';
		} 
		if($('#_exemptionValue').val() == '') {
			errorsFound = true;
			errMsg += '<li><spring:message code="field.errors.generic.required" arguments="Exemption Value"/></li>';
		} 
		if(!errorsFound) {
			if("PTY_ID"===$('#_exemptionType').val() || "WRK_ID"===$('#_exemptionType').val() || "PROVIDER_ID"===$('#_exemptionType').val() ) {
				if($('#_exemptionValue').val().length > 15) {
					errorsFound = true;
					errMsg += '<li><spring:message code="error.apm.exemption.value.digits.15"/></li>';
				}	
			}
			else if("LIBRARY_DISC_ID"===$('#_exemptionType').val() ) {
				if($('#_exemptionValue').val().length > 20) {
					errorsFound = true;
					errMsg += '<li><spring:message code="error.apm.exemption.value.length.20"/></li>';
				}	
			}
			else if("WRK_TTL"===$('#_exemptionType').val() ) {
				if($('#_exemptionValue').val().length > 150) {
					errorsFound = true;
					errMsg += '<li><spring:message code="error.apm.exemption.value.length.150"/></li>';
				}	
			}
			else if("WRITER"===$('#_exemptionType').val() ) {
				if($('#_exemptionValue').val().length > 100) {
					errorsFound = true;
					errMsg += '<li><spring:message code="error.apm.exemption.value.length.100"/></li>';
				}	
			}
		}
		if(!errorsFound) {
			if("PTY_ID"===$('#_exemptionType').val() || "WRK_ID"===$('#_exemptionType').val() || "PROVIDER_ID"===$('#_exemptionType').val() ) {
				...
				if(isNaN(exVal) || Number(exVal) <=0 || !allNumeric(exVal)) {
					...
					if("PTY_ID"===$('#_exemptionType').val()) {
						errMsg += '<li><spring:message code="field.errors.generic.numeric" arguments="Exemption Value"/></li>';
					}
...
<table class="detailstable" style="width:50%;margin:auto;">
		...
		<tr>
			...
			<c:if test="${exemption.operationType ne 'RETRIEVE'}">
				<form:select path="exemptionType" styleId="_exemptionType">	
					<form:option value="" />
					<form:options items="${exemptionTypes}" itemLabel="displayName" itemValue="id" />
				</form:select>	
			</c:if>
