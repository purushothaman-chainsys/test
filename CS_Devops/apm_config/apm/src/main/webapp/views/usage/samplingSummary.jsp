...
function submitForm(val1, val2, val3, obj) {
	...
	if(playCntRange == '') {
		...
		$("#uierror").html('<span class=txtRed><li><spring:message code="error.apm.sampling.playcountranges.required"/></span>');
		unCheckSub(obj);
	 	location.href="#"; 
	 	return;
	}
		...
		if(!allNumeric(playCntRange) ||  isNaN(playCntRange) || Number(playCntRange) <= 0 ) {
			...
			$("#errorMessages").html("");
			$("#serverErrorMessages").html("");
			$("#uierror").html('<span class=txtRed><li><spring:message code="error.apm.sampling.playcountranges.numeric"/></span>');
			unCheckSub(obj);
		 	location.href="#"; 
		 	return;
		}
...
<c:if test="${sampleRequest.manMatchIndicator ne 'Y'}"> 
			...
			<c:if test="${sampleRequest.samplingStatusCode eq 'PE'}"> 		
				...
				<c:if test="${sampleRequest.samplingStatusCode ne 'CO'}"> 
					...
					<c:if test="${sampleRequest.samplingStatusCode ne 'BD'}"> 
					Completed	
					</c:if>		
					</c:if>
...
<c:if test ="${sampleRequest.manMatchIndicator ne 'Y'}">
				 ...
				 <c:if test ="${sampleRequest.statusCode eq 'PE'}">
					...
					&nbsp;&nbsp;
					<input type="text" value="${sampleRequest.playCountRanges}" size="3" maxlength="3"/><% playCountRowNum++; %> 
					</c:if>
