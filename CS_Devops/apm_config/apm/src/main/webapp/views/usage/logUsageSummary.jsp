...
function add(obj) {    
   ...
   if(!widValidNumeric) {	
		...
		$("#errorMessages").html("");
		$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.logusage.insert.widnumericerr"/></span>'); 
		unCheckSub(obj);
	 	location.href="#"; 
	 	return;
	}
	if(!durationValidNumeric) {	
		$("#serverErrorMessages").html("");
		$("#errorMessages").html("");
		$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.logusage.insert.durationnumericerr"/></span>'); 
		unCheckSub(obj);
	 	location.href="#"; 
	 	return;
	}
	if(!playsValidNumeric) {	
		$("#serverErrorMessages").html("");
		$("#errorMessages").html("");
		$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.logusage.insert.playsnumericerr"/></span>'); 
		unCheckSub(obj);
	 	location.href="#"; 
	 	return;
	}
...
function del(obj) {  
		...
		$("#serverErrorMessages").html("");
		$("#errorMessages").html("");
		$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.logusage.delete.selection"/></span>');
		unCheckSub(obj);
	 	location.href="#"; 
	 	return;
	    }
		...
		var logstatus = $(this).closest('tr').find('input[name=status]').val();  
		 ...
		 if(nodelete) {	
			$("#serverErrorMessages").html("");
			$("#errorMessages").html("");
			$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.logusage.update.statuserr"/></span>');
			unCheckSub(obj);
		 	location.href="#"; 
		 	return;
		  }
...
function update(obj) {   
	...
	$("input[name=updInstvocals]").each(function(){
	 ...
	 if(!validNumeric) {	
		$("#serverErrorMessages").html("");
		$("#errorMessages").html("");
		$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.logusage.insert.numericerr"/></span>');
		unCheckSub(obj);
	 	location.href="#"; 
	 	return;
	}
...
function logrequest(obj) {    
			...
			document.forms[0].action = '<%=request.getContextPath()%>/usage/logrequestSummary.do';
...
<form:form action="logusage" modelAttribute="logUsageForm">
<table class="errortable">
		<tr>
			<td>
				<div id="serverErrorMessages">
				<span class="txtGreen"><form:errors path="${systemmessage}" /></span>
				<span class="txtRed"><form:errors path="${systemerror}" /></span>
				</div>
				<div id="uierror"></div>
...
<tr>
	<td>Call Letter: ${logUsageForm.callLetter}</td>
	<td>Start Date: ${logUsageForm.startDate}</td>
	<td>End Date: ${logUsageForm.endDate} </td>
	<td>Account Number: ${logUsageForm.accountNumber}</td>
</tr>
...
<c:if test="${logUsageForm.searchRequested eq 'Y'}"> 
<table class="detailstable1000" style="width:1200px; font-size: 11px;color: #000000; line-height: 11px; margin:0 auto;"> 
	<tr > 
	
	<td align="left">Found ${logUsageForm.searchResultRows} Results 
	</td>	
	
	<td align="left" width="15%" > 
	 
		 <c:if test="${logUsageForm.totalPg ne '0'}"> 
			Page ${logUsageForm.currentPg} of  
			${logUsageForm.totalPg} </c:if>
		</td>
		<td align="left" width="25%" >
		
		<c:if  test="${logUsageForm.totalPg ne '0'}">
		
		<c:if  test="${logUsageForm.totalPg ne '1'}">
		
			Page #&nbsp; 
			<form:input path="pageNumber" size="1" maxlength="5"  value='<c:out value="${logUsageForm.currentPg}"/>' onkeyup="javascript:resetPageNumber(this.value);"></form:input>
			<prep:uiWidget type="${com.ascap.apm.common.utils.constants.SecurityConstants.ICON_TYPE}" 
			title="${com.ascap.apm.common.utils.constants.UIWidgetConstants.LABEL_NAV_GO}" 
			id="${com.ascap.apm.common.utils.constants.UIWidgetConstants.ID_NAV_GO}">
			onclick="javascript:setPageNumber();"</prep:uiWidget>
			 
			<c:if  test="${logUsageForm.firstPg eq 'N'}"> 
			...
			<c:if  test="${logUsageForm.lastPg eq 'N'}"> 
			<prep:uiWidget type="${com.ascap.apm.common.utils.constants.SecurityConstants.ICON_TYPE}" 
			title="${com.ascap.apm.common.utils.constants.UIWidgetConstants.LABEL_NAV_NEXT}" 
			id="${com.ascap.apm.common.utils.constants.UIWidgetConstants.ID_NAV_NEXT}">
...
<tbody>
 ...
 <% int sampleRowNum = 0; %>  	 	 
<c:if test="${not empty logUsageForm.searchResults}">
<c:forEach varStatus="currentIndexId" var="logUsageResult" items="${logUsageForm.searchResults}">
<% String useTypeTemp = ""; %> 	
