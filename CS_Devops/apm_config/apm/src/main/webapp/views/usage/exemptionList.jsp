...
function filter() {
		if($('#_filterExemptionType').val()=='') {
			...
			$("#serverErrorMessages").html("");
			$("#uierror").html("");
			$("#uierror").html('<span class=txtRed><li><spring:message code="error.apm.exemption.filter.required"/></li></span>');
		 	location.href="#"; 
		 	return;
		}
		document.forms[0].operationType.value = 'SEARCH';
...
<table class="detailstable1000" style="width:1000px; font-size: 11px; font-weight: bold; color: #000000; line-height: 11px; margin:0 auto;"> 
	<tr>	
		<td>Exemption Type<br><form:select path="filterExemptionType" styleId="_filterExemptionType">
						<form:option value="" />
