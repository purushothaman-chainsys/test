	...
	$("input[name=medleyWorkIds]").each(function(){
			...
			if($.trim(wId) === '' ) {
				...
				if(isNaN(wId) || Number(wId) <=0 || !allNumeric(wId)) {
						workIdNumeric = false;
						errMsg += '<li><spring:message code="error.apm.archives.workid.nonnumber"/>';
					}
				}
		});
...
&nbsp;<form:input path="medleyCloneCounts[${currIndexId.index}]"  size="1" maxlength="2"  readonly="true"/>
<% } else { %>
<input type="hidden" name="medleyWorkOriginalInd" value="N"/>
<input  name="medleyWorkIds" value="${_wIdTemp}" />
&nbsp;<form:input path="medleyCloneCounts[${currIndexId.index}]"  size="1" maxlength="2"/>
<% } %>
...
<td><form:input  path="medleyWorkIds"/>
