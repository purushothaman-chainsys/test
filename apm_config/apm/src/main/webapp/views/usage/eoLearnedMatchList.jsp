...
$(function() {
	...
	$('a[id=_Match_to_Multiple_Works]').click(function() {
		...
		if($("table#mainData tr input[name='selectedIndex']:checked").length > 1) {    			
			...
			$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.manualmatch.multiplematch.selection.gt1"/></span>'); 
			location.href="#";
			return;
		}
		else if($("table#mainData tr input[name='selectedIndex']:checked").length < 1) {    			
			$("#serverErrorMessages").html("");
			$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.manualmatch.multiplematch.selection.lt1"/></span>'); 
			location.href="#";
			return;
		}
		else if($("table#mainData tr input[name='selectedIndex']:checked").length == 1){
	...
	$('a[id^=_Delete]').click(function() {
			...
			if(nonMultRow && multRow) {
				...
				errorsFound = true;
				$("#serverErrorMessages").html("");
				$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.learnedmatch.delete.mixrows"/></span>');				
			...
			if($("table#mainData tr input[name='selectedIndex']:checked").length > 0) {	
	    		...
	    		$("#serverErrorMessages").html("");
				$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.archives.delete.selection"/></span>'); 
				location.href="#";
				return;
	    	}
	    	//}
...
function update(obj) {
	...
	if(multRow) {
		$("#serverErrorMessages").html("");
		$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.learnedmatch.update.mixrows"/></span>');				
		unCheckSub(obj);
		location.href="#";
		return;
	}
	...
	$("input[name=selectedIndex]:checked").each(function(){
			...
			if($(this).closest('tr').find('input[name=workId]').val() == '') {
				...
				if(isNaN(wId) || Number(wId) <=0 || !allNumeric(wId)) {
					workIdNumeric = false;
					errMsg += '<li><fmt:message key="error.apm.archives.workid.nonnumber"><fmt:param value="${wId}"/></fmt:message></li>';
				}
			}
	...
	if(!workIdExists) {		
			$("#errorMessages").html("");
			$("#serverErrorMessages").html("");
			$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.archives.workid.required"/></li></span>');
			unCheckSub(obj);
		 	location.href="#"; 
		 	return;
	}
...
function filter() {
	...
	if($('#_filterMatchTypeCode').val()=='') {
			...
			$("#serverErrorMessages").html("");
			$("#uierror").html("");
			$("#uierror").html('<span class=txtRed><li><fmt:message key="error.apm.eo.learnedmatch.filter.required"/></li></span>');
			//unCheckSub(obj);
		 	location.href="#"; 
		 	return;
		}
			...
			if($.trim($("input[name=filterWorkId]").val()) == '') {				
				...
				if(isNaN(fwId) || Number(fwId) <=0 || !allNumeric(fwId)) {
					...
					ferrMsg += '<li><fmt:message key="error.apm.archives.workid.nonnumber"><fmt:param value="${fwId}"/></fmt:message>';
				}
			}
			...
			if($.trim($("input[name=filterMultWorkId]").val()) == '') {				
				...
				if(isNaN(fmwId) || Number(fmwId) <=0 || !allNumeric(fmwId)) {
					...
					ferrMsg += '<li><fmt:message key="error.apm.archives.multworkid.nonnumber"><fmt:param value="${fmwId}"/></fmt:message>';
				}
			}
 ...
 <table class="errortable" style="width:1000px;">
		<tr>
			<td>
				...
				<span class="txtGreen"><c:out value="${systemmessage}"></c:out></span>
				<span class="txtRed"><c:out value="${systemerror}"></c:out></span>
				</div>
				<div id="uierror"></div>
...
<form:form action="apmLearnedMatchEO" modelAttribute="eoLearnedMatchListForm"> 
...
<c:if test="${eoLearnedMatchListForm.filterMatchTypeCode eq 'URL'}">
...
<c:if test="${eoLearnedMatchListForm.filterMatchTypeCode eq 'SUPPLIERID'}">
<prep:uiWidget name="<%= ProtectedResourcesConstants.APM_LM_100_LEARNED_MATCH_UPDATE%>" type="<%= SecurityConstants.ANCHOR_TYPE %>" label="<%=UIWidgetConstants.LABEL_MATCH_MULTIPLE_WORKS %>" id="<%=UIWidgetConstants.ID_MATCH_MULTIPLE_WORKS%>" title="Match to Multiple Works">onclick="javascript:void(0);"</prep:uiWidget>
</c:if>
 </td>
...
<c:if test="${eoLearnedMatchListForm.filterMatchTypeCode eq 'ISRC'}">
	<th width="25%">ISRC</th> 
	</c:if>
<c:if test="${eoLearnedMatchListForm.filterMatchTypeCode eq 'ISWC'}">
	<th width="25%">ISWC</th> 
	</c:if>
<c:if test="${eoLearnedMatchListForm.filterMatchTypeCode eq 'URL'}">
	<th width="25%">Url</th> 
...
<c:if test="${eoLearnedMatchListForm.filterMatchTypeCode eq ''}">
	<th width="25%"></th> 
	</c:if>
</tr>
</thead>
<tbody>
  	...
  	<c:if test="${not empty eoLearnedMatchListForm.searchResults}">
 <c:forEach var="apmPerformanceBulkRequest" items= "${eoLearnedMatchListForm.searchResults}" varStatus="currentIndexId">
 <c:choose>
...
<input type="hidden"  value="${apmPerformanceBulkRequest.supplierCode}" id='supplierCode_${currentIndexId.index}'/>
</td>
<td align="left">
...
<c:if test="${eoLearnedMatchListForm.filterMatchTypeCode ne 'URL'}">
<c:out value="${apmPerformanceBulkRequest.matchType}"></c:out>
</c:if>
<input type="hidden"  value="${apmPerformanceBulkRequest.matchType}" id='matchType_${currentIndexId.index}'/>
</td>
<c:if test="${eoLearnedMatchListForm.filterMatchTypeCode eq 'URL'}">
<td align="left"><c:out value="${apmPerformanceBulkRequest.multWorkId}"></c:out></td>
<input type="hidden"  value="${apmPerformanceBulkRequest.multWorkId}" id='multWorkId_${currentIndexId.index}'/>
</c:if>
