...
<%
com.ascap.apm.vob.usage.EOLearnedMatchForm xApmLearnedMatchForm;
...
try{
	xApmLearnedMatchForm =(com.ascap.apm.vob.usage.EOLearnedMatchForm) request.getAttribute("eoLearnedMatchForm");
...
$(function() {
	...
	$('#_Update').click(function() {		
		...
		if($('#_matchIdValue').val() == '') {
			errorsFound = true;
			errMessage += '<li><spring:message code="error.apm.eo.learnedmatch.requiredfields.matchvalue"/>';
		}
		else {
			$('#_matchIdValue').val($.trim($('#_matchIdValue').val()));
			if($('#_matchIdValue').val().indexOf(' ') >= 0) {
				errorsFound = true;
				errMessage += '<li><spring:message code="error.apm.eo.learnedmatch.url.spacechar"/>';
			}
		}
		
		if($('input[name=workIds]').length < 1) {
			errorsFound = true;
			errMessage += '<li><spring:message code="error.apm.eo.learnedmatch.requiredfields.workid"/>';
		}
		...
		$("input[name=workIds]").each(function(){
			...
			if($.trim(wId) === '' ) {
				if ($("#_learnTypeFLD").val() == 'N') {
					errorsFound = true;	
					if(!workIdMissing) {
						errMessage += '<li><spring:message code="error.apm.archives.workid.required"/>';			
						workIdMissing = true;
					}
				}
			}
				...
				if(isNaN(wId) || Number(wId) <=0 || !allNumeric(wId)) {
						errorsFound = true;
						errMessage += '<li><spring:message code="error.apm.archives.workid.nonnumber" arguments="'+wId+'"/>';
					}
				}
		});
		...
		$("input[name=medleyCloneCounts]").each(function(){
			...
			if(isNaN(cloneValue) || Number(cloneValue) <=0 || !allNumeric(cloneValue)) {
				countsNumeric = false;
				errorsFound = true;
				errMessage += '<li><spring:message code="error.apm.manualmatch.multiplematch.clonecount.numeric"/>';
			}
			if(countsNumeric) {
				if(Number(cloneValue) > 20) {
					errorsFound = true;
					errMessage += '<li><spring:message code="error.apm.manualmatch.multiplematch.clonecount.maxlimit"/>';
				}
			}
	...
	$('#_Submit').click(function() {
		...
		if( $("#_matchType").val() != 'URL' && $("#_matchType").val() != 'SUPPLIERID') {
			...
			if($('input[name=workIds]').length != 1) {
				errorsFound = true;
				errMessage += '<li><spring:message code="error.apm.eo.learnedmatch.workid.gt1"/>';
			}
			...
			if( $("#_matchType").val() === 'URL') {
				if($('#_matchIdValue').val().toLowerCase().indexOf("<%=com.ascap.apm.common.utils.constants.UsageConstants.YOUTUBE_URL_PREFIX %>") != 0) {
					errorsFound = true;
					errMessage += "<li><spring:message code='error.apm.learnedmatch.url.format.invalid'/>";
				}
				else {
					if($.trim($('#_matchIdValue').val()).length <= "<%=com.ascap.apm.common.utils.constants.UsageConstants.YOUTUBE_URL_PREFIX %>".length){
						errorsFound = true;
						errMessage += "<li><spring:message code='error.apm.learnedmatch.url.videoid.invalid'/>";
					}
				}
			}
				...
				if($.trim($('#_matchIdValue').val()).length == 0) {
					//errorsFound = true;
					//errMessage += "<li><fmt:message key='error.apm.eo.learnedmatch.requiredfields.matchvalue'/>";
				}
			}
		...
		if($.trim($('#_supplierCode').val()) == '' && $("#_matchType").val() === 'SUPPLIERID') {
			errorsFound = true;
			errMessage += '<li><spring:message code="error.apm.eo.learnedmatch.requiredfields.supplier"/>';
		}
		if($('#_matchType').val() == '') {
			errorsFound = true;
			errMessage += '<li><spring:message code="error.apm.eo.learnedmatch.requiredfields.matchtype"/>';
		}
		if($('#_matchIdValue').val() == '' ) {
			errorsFound = true;
			errMessage += '<li><spring:message code="error.apm.eo.learnedmatch.requiredfields.matchvalue"/>';
 ...
 <table class="errortable" style="width:600px;">
		<tr>
			<td>
				...
				<span class="txtRed" style="align:left;"><c:out value="${systemerror}" /></span>
				</div>
				<div id="uierror"></div>
...
<form:form action="learnedMatchDetailsEO" modelAttribute="eoLearnedMatchForm">

<% boolean isDisabled = false; %>
<c:if test="${eoLearnedMatchForm.operationType eq 'EDIT' }">
<%  isDisabled = true; %>
</c:if>
...
<c:if test="${eoLearnedMatchForm.operationType ne 'EDIT' }">
