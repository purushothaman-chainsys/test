	...
	<table class="errortable">
		<tr>
			<td width="100%">
				...
				<c:out value="${systemmessage}" /></span>
				<span class="txtRed"><c:out value="${systemerror}" />
				<form:errors path="maxSearchResults"></form:errors>
				<form:errors path="nofSrchRsltsPerPage"></form:errors>
				</span>
			</td>
		</tr>
	</table>
		...
		<tr>
			...
			<c:if test ="${sessionScope[UserSessionState.HTTP_SESSION_KEY].userPreference.themeTypeCode  eq themeTypeCode }">
					<c:if test="${not empty sessionScope[UserSessionState.HTTP_SESSION_KEY].userPreference.randomThemeTypeCode }">
					 <c:set var ="randomThemeDes" value="${sessionScope[PREP.UserSessionState].userPreference.randomThemeTypeCode}"/>
						<span class="txtBlk">			
