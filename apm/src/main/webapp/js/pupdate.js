...
function formatDate(date,format) {
	...
	var c="";
	var token="";
	var y=padNumber(date.getFullYear(),4)+"";
...
function getDateFromFormat(val,format) {
	...
	var x,y;
	var now=new Date();
	var year=padNumber(now.getFullYear(),4);
...
function PopupWindow() {
	...
	this.contents = "";
	this.url="";
	this.windowProperties="toolbar=no,location=no,status=no,menubar=no,scrollbars=auto,resize=no,alwaysRaised,dependent,titlebar=no";
...
function CP_tmpReturnFunction(y,m,d) { 
	if (window.CP_targetInput!=null) {
		var dt = new Date(padNumber(y,4),m-1,d,0,0,0);
		dt.setFullYear(y);
...
function CP_getCalendar() {
			...
			for (var col=1; col<=7; col++) {
				...
				if (disabled || this.disabledWeekDays[col-1]) {
					...
					if (this.displayType=="week-end") {
						...
						selected_year = padNumber(d.getFullYear(),4);
	...
	if (this.displayType=="month" || this.displayType=="quarter" || this.displayType=="year") {
		if (arguments.length > 0) { var year = arguments[0]; }
			...
			if (this.displayType=="year") {	var year = padNumber(now.getFullYear()-this.yearSelectStartOffset,4); }
			else { var year = padNumber(now.getFullYear(),4); }
			}
