...
$(function() {
	...
	$('a[id^=_Assign]').click(function() {
        ...
        var userNotAllowed = false;
					...
					if (   $(this).closest('tr').find('input[name=assignedToUser]').val()  !=  '<c:out value="${apmPerformanceBulkRequestList.userId}"/>') {
					   userNotAllowed = true; 
					  } 
				}); 
				...
				if(userNotAllowed) {
					...
					$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.manualmatch.assign.selection.notvalid"/></span>'); 
					location.href="#";
					return;
				}
		}
	...
	if($("table#mainData tr input[name='selectedIndex']:checked").length > 0) {	}
	   		...
	   		$("#serverErrorMessages").html("");
			$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.manualmatch.assign.selection.none"/></span>'); 
			location.href="#";
			return;
		}
		...
		if($('select[name="assignUser"]').val() === '') {
			$("#serverErrorMessages").html("");
			$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.manualmatch.asgusrid.value.none"/></span>'); 
			location.href="#";
			return;
		}
        resetFinalValues();
	...
	$('a[id^=_Unassign]').click(function() {
    	...
    	$('a[id^=_Delete]').click(function() {
			...
			if($("table#mainData tr input[name='selectedIndex']:checked").length > 0) {	
				...
				if(! userAllowed) {
					$("#errorMessages").html("");
					$("#serverErrorMessages").html("");
					$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.manualmatch.assign.rows.selection.invalid"/></li></span>');
		       	...
		       	if(!validForDelete) {
					$("#errorMessages").html("");
					$("#serverErrorMessages").html("");
					$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.archives.originalworkid.exists"/></span>');
    		...
    		if($("table#mainData tr input[name='selectedIndex']:checked").length > 1) {    			
    			...
    			$("input[name=selectedIndex]:checked").each(function(){    				
    				...
    				if(! isUserAllowed(cIndex)) {				
						allowedUser = false;
						errMsgInvalidUser = '<li><spring:message code="us.error.apm.manualmatch.assign.rows.selection.invalid"/></li>';
					}
				});
    			if(!allowedUser) {
					$("#errorMessages").html("");
				...
				if( $.trim(  $("input[name=multWorkId]:eq("+checkedIndex+")").val()) == '') {
						if( $.trim(  $("input[name=originalWorkId]:eq("+checkedIndex+")").val()) != '') {
							$("#serverErrorMessages").html("");
							$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.multiploematch.singleworkid.exists"/></span>'); 
							location.href="#";
							return;
						}
					}
    	...
    	$('a[id^=_Unmatch]').click(function() {
			...
			$("input[name=selectedIndex]:checked").each(function(){
				...
				if( $.trim(  $("input[name=multWorkId]:eq("+checkedIndex+")").val()) != '') {
					...
					if((($("input[name=originalWorkId]:eq("+checkedIndex+")").val()) == '')) {
						...
						$("#errorMessages").html("");
						$("#serverErrorMessages").html("");
						$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.archives.originalworkid.required"/></span>');
						unCheckSub($('a[id^=_Unmatch]'));
			 			location.href="#"; 
			 			return;
					}
			...
			if(validForUnmatch) {
	    		...
	    		$("#serverErrorMessages").html("");
				$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.archives.unmatch.selection.none"/></span>'); 
				location.href="#";
				return;
	    	}
	    	else if($("table#mainData tr input[name='selectedIndex']:checked").length > 1) {
	    		$("#serverErrorMessages").html("");
				$("#uierror").html('<span class=txtRed><li><spring:message code="us.error.apm.archives.unmatch.selection.multiple"/></span>'); 
				location.href="#";
				return;
	    	}
    	});
		...
		$("input[name=selectedIndex]:checked").each(function(){
			...
			if($.trim($("input[name=workId]:eq("+checkedIndex+")").val()) == '') {
				...
				if(isNaN(wId) || Number(wId) <=0 || !allNumeric(wId)) {
					workIdNumeric = false;
					errMsg += '<li><spring:message code="us.error.apm.archives.workid.nonnumber" arguments="'+wId+'"/></li>';
				}
			}
			if( $.trim(  $("input[name=multWorkId]:eq("+checkedIndex+")").val()) != '') {
				matchedToMultipe = true;
				errMsgMult += '<li><spring:message code="us.error.apm.multiplematch.update.invalid"/></li>';
			}
...
function isUserAllowed(checkedIndex) {
	if('N' === $("input[name=canUpdateOthers]").val())   {
		if( $.trim(  $("input[name=assignedToUser]:eq("+checkedIndex+")").val()) != '' ) {
			if( $.trim(  $("input[name=assignedToUser]:eq("+checkedIndex+")").val()) != '<c:out value="${apmPerformanceBulkRequestList.userId}"/>') {
				return false
			}
		}	
	}
...
function addRows()	{
	$("#mdlTbl>tbody").append('<tr align="center"><td align="center"><input type="checkbox" name="abcdef_chk"></input></td><td align="center"><input type="text" name="medleyWorkIds" /></td></tr>');		
}
function deleteRows() {
...
<table class="detailstable1000" style="width:1200px; font-size: 11px; font-weight: bold; color: #000000; line-height: 11px; margin:0 auto;">	 
	<tr>	
		<td>Supplier<br><form:select path="filterSupplierCode" styleId="filterSupplier">
			<form:option value="" />
		...
		<td>Group By Supplier<br><form:select path="filterResultViewType" style="width:120px;" id="groupByFilter"><form:option value="SPLR">Yes</form:option><form:option value="NSPLR">No</form:option></form:select></td>
		<td>Assigned To<br/>
...
<form:hidden path="nonMasteruser" value="Y"/>
</prep:hasPermission>
</prep:hasPermission>
 <prep:hasPermission name="<%= ProtectedResourcesConstants.APM_WM_100_WRK_MATCH_ASSIGN%>" value="N">
...
<c:forEach varStatus="currentIndexId" var="apmPerformanceBulkRequest" items="${apmPerformanceBulkRequestList.searchResults}" >		
<c:choose>
<c:when test="${currentIndexId.index % 2 eq 0 }">
<tr class="o20">
</c:when>
<c:otherwise>
<tr>
...
<c:if test="${apmPerformanceBulkRequest.deleteFlag  eq 'Y'}">
   <c:if  test="${apmPerformanceBulkRequestList.filterMatchStatusCode ne 'DEL'}">
	  <form:checkbox path="selectedIndex" style="padding-top:0px;" disabled="true"  value="${currentIndexId.index}"></form:checkbox> 
   </c:if>
</c:if>
<c:if test="${apmPerformanceBulkRequest.deleteFlag eq 'Y'}">
   <c:if test="${apmPerformanceBulkRequestList.filterMatchStatusCode eq 'DEL'}">
	  <form:checkbox path="selectedIndex" style="padding-top:0px;" disabled="false" value="${currentIndexId.index}"></form:checkbox>
   </c:if>
</c:if>
<c:if test="${apmPerformanceBulkRequest.deleteFlag ne 'Y'}">
	<input type="checkbox" name="selectedIndex" style="padding-top:0px;" value="${currentIndexId.index}"/> 
</c:if>		
	<form:hidden path="requestId"></form:hidden>
</td>
...
<td align="right"><c:out value="${apmPerformanceBulkRequest.workPerfCount}" />&nbsp;&nbsp;&nbsp;&nbsp;</td>
<td align="right"><c:out value="${apmPerformanceBulkRequest.multWorkId}" />&nbsp;&nbsp;&nbsp;&nbsp;</td>
...
<c:out value="${apmPerformanceBulkRequest.estimatedDollarAmount}" />
</td>
...
<input type="hidden"  name="originalWorkId" value ="${apmPerformanceBulkRequest.originalWorkId}" Id='"originalWorkId_"+${currentIndexId.index}'/>
...
<form:hidden path="estimatedDollarAmount[${currentIndexId.index}]" value="${apmPerformanceBulkRequest.estimatedDollarAmount}" styleId='"estimatedDollarAmount_"+${currentIndexId.index}'/>
...
<input type="hidden" name="groupingSubpplierCode" value="<c:out value="${apmPerformanceBulkRequestList.filterSupplierCode}"/>"/>
 ...
 <table class="searchpagestatus" style="width: 1200px; ">
	<tr class="searchpagestatus" style="width: 1200px; ">
		<td align="right">Results found so far <b><c:out value="${apmPerformanceBulkRequestList.numberOfRecordsFound}" /></b></td>
	</tr>
</table>
